...
if ($clube_id) {
    ...
    $stmt = $pdo->prepare("SELECT j.*, cc.nome as clube_casa_nome, cc.logo as clube_casa_logo, cc.codigo as clube_casa_codigo, cf.nome as clube_fora_nome, cf.logo as clube_fora_logo, cf.codigo as clube_fora_codigo FROM jogos j JOIN clubes cc ON j.clube_casa_id = cc.id JOIN clubes cf ON j.clube_fora_id = cf.id WHERE (j.clube_casa_id = ? OR j.clube_fora_id = ?) AND j.status = 'agendado' ORDER BY j.data_jogo ASC LIMIT 10");
    $stmt->execute([$clube_id, $clube_id]);
    $proximosJogos = $stmt->fetchAll();
    
    $stmt = $pdo->prepare("SELECT j.*, cc.nome as clube_casa_nome, cc.logo as clube_casa_logo, cc.codigo as clube_casa_codigo, cf.nome as clube_fora_nome, cf.logo as clube_fora_logo, cf.codigo as clube_fora_codigo FROM jogos j JOIN clubes cc ON j.clube_casa_id = cc.id JOIN clubes cf ON j.clube_fora_id = cf.id WHERE (j.clube_casa_id = ? OR j.clube_fora_id = ?) AND j.status = 'finalizado' ORDER BY j.data_jogo DESC LIMIT 10");
    $stmt->execute([$clube_id, $clube_id]);
